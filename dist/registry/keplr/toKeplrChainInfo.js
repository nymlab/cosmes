const getRpcEndpoint = (chain) => chain.apis?.rpc?.[0]?.address ?? "";
const getRestEndpoint = (chain) => chain.apis?.rest?.[0]?.address ?? "";
/**
 ...
 */
export function toKeplrChainInfo(chainRegistryChainInfo, chainRegistryAssetList, options = {}) {
    // Adapted from: https://github.com/cosmology-tech/chain-registry/blob/main/packages/keplr/src/index.ts
    const currencies = chainRegistryAssetList.assets.map((asset) => ({
        coinDenom: asset.symbol,
        coinMinimalDenom: asset.base,
        coinDecimals: asset.denom_units.find((denomUnit) => denomUnit.denom === asset.display)?.exponent ?? 6,
     ...
     */
    let stakeCurrency = currencies[0];
    if (chainRegistryChainInfo.staking) {
        for (const token of chainRegistryChainInfo.staking.staking_tokens) {
            const currency = currencies.find((currency) => currency.coinDenom === token.denom);
            if (currency) {
     ...
     */
    const gasPriceSteps = chainRegistryChainInfo.fees?.fee_tokens?.reduce((m, feeToken) => {
        m[feeToken.denom] = {
    ...
    const feeCurrencies = currencies
        .filter((currency) => chainRegistryChainInfo.fees?.fee_tokens.find((t) => t.denom === currency.coinMinimalDenom))
        .map((feeCurrency) => {
...
//# sourceMappingURL=toKeplrChainInfo.js.map